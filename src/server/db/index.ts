...
function readConfigFile(): any {
  const p = resolve(process.cwd(), 'i18n-dashboard.config.json')
  if (existsSync(p)) {
...
function buildConnection(config: ReturnType<typeof useRuntimeConfig>): Knex.Config {
  ...
  const params = {
    dbClient: (override?.dbClient || fileConfig?.database?.client || config.dbClient || 'better-sqlite3') as string,
    dbConnection: (override?.dbConnection || fileConfig?.database?.connection || config.dbConnection) as string | undefined,
    dbHost: (override?.dbHost || fileConfig?.database?.host || config.dbHost) as string | undefined,
    dbPort: (override?.dbPort || fileConfig?.database?.port || config.dbPort) as string | undefined,
    dbUser: (override?.dbUser || fileConfig?.database?.user || config.dbUser) as string | undefined,
    dbPassword: (override?.dbPassword || fileConfig?.database?.password || config.dbPassword) as string | undefined,
    dbName: (override?.dbName || fileConfig?.database?.name || config.dbName) as string | undefined,
  }
...
const DASHBOARD_UI_STRINGS: Array<{ key: string; fr: string; en: string }> = [
  ...
  { key: 'onboarding.project_name_path_required', fr: 'Le nom et le chemin du projet sont requis.', en: 'Project name and path are required.' },
  { key: 'onboarding.project_name_required', fr: 'Le nom du projet est requis.', en: 'Project name is required.' },
  { key: 'onboarding.project_path_required', fr: 'Le chemin du projet est requis.', en: 'Project path is required.' },
  { key: 'onboarding.project_git_url_required', fr: "L'URL du dépôt est requise.", en: 'Repository URL is required.' },
  { key: 'onboarding.source_local', fr: 'Chemin local', en: 'Local path' },
  { key: 'onboarding.source_git', fr: 'Dépôt Git', en: 'Git repository' },
  ...
  { key: 'logs.title', fr: 'Logs système', en: 'System logs' },
  { key: 'logs.description', fr: 'Logs d\'erreurs et d\'avertissements générés par les processus en arrière-plan', en: 'Error and warning logs generated by background processes' },
  { key: 'logs.settings_title', fr: 'Paramètres de purge automatique', en: 'Auto-purge settings' },
  { key: 'logs.retention_days_label', fr: 'Conserver les logs pendant (jours)', en: 'Retain logs for (days)' },
  { key: 'logs.purge_interval_label', fr: 'Intervalle de purge automatique (heures)', en: 'Auto-purge interval (hours)' },
  { key: 'logs.purge_all', fr: 'Tout purger', en: 'Purge all' },
  { key: 'logs.empty', fr: 'Aucun log trouvé', en: 'No logs found' },
  { key: 'logs.filter_context', fr: 'Filtrer par contexte...', en: 'Filter by context...' },
  { key: 'logs.show_details', fr: 'Détails', en: 'Details' },
  { key: 'logs.purged_toast', fr: 'Logs purgés', en: 'Logs purged' },
  { key: 'logs.entries_deleted', fr: 'entrées supprimées', en: 'entries deleted' },
  { key: 'logs.level_all', fr: 'Tous les niveaux', en: 'All levels' },
  { key: 'logs.level_error', fr: 'Erreur', en: 'Error' },
  { key: 'logs.level_warn', fr: 'Avertissement', en: 'Warning' },
  { key: 'logs.level_info', fr: 'Info', en: 'Info' },
  { key: 'logs.nav_label', fr: 'Logs système', en: 'System logs' },
  { key: 'common.refresh', fr: 'Rafraîchir', en: 'Refresh' },
  { key: 'smtp.title', fr: 'Configuration SMTP', en: 'SMTP configuration' },
  { key: 'smtp.description', fr: 'Configurez l\'envoi d\'emails pour les invitations et la réinitialisation de mot de passe', en: 'Configure email sending for invitations and password resets' },
  { key: 'smtp.host_label', fr: 'Hôte SMTP', en: 'SMTP host' },
  { key: 'smtp.port_label', fr: 'Port', en: 'Port' },
  { key: 'smtp.secure_label', fr: 'Connexion sécurisée (TLS/SSL)', en: 'Secure connection (TLS/SSL)' },
  { key: 'smtp.user_label', fr: 'Nom d\'utilisateur', en: 'Username' },
  { key: 'smtp.pass_label', fr: 'Mot de passe', en: 'Password' },
  { key: 'smtp.from_label', fr: 'Adresse expéditeur', en: 'From address' },
  { key: 'smtp.dashboard_url_label', fr: 'URL du dashboard', en: 'Dashboard URL' },
  { key: 'smtp.dashboard_url_hint', fr: 'Utilisé pour les liens dans les emails', en: 'Used for links in emails' },
  { key: 'smtp.save', fr: 'Enregistrer', en: 'Save' },
  { key: 'smtp.test', fr: 'Tester', en: 'Test' },
  { key: 'smtp.test_email_label', fr: 'Envoyer un email de test à', en: 'Send test email to' },
  { key: 'smtp.not_configured', fr: 'SMTP non configuré — les emails ne seront pas envoyés', en: 'SMTP not configured — emails will not be sent' },
  { key: 'smtp.test_sent', fr: 'Email de test envoyé', en: 'Test email sent' },
  { key: 'smtp.test_failed', fr: 'Échec de l\'envoi', en: 'Send failed' },
  { key: 'smtp.saved', fr: 'Configuration SMTP enregistrée', en: 'SMTP configuration saved' },
  { key: 'smtp.nav_label', fr: 'SMTP', en: 'SMTP' },
]
...
export async function initDb(): Promise<void> {
  ...
  // ── smtp settings (seed from config file or env vars, only if not yet set) ──
  const _fileConfig = readConfigFile()
  const _smtp = _fileConfig?.smtp || {}
  const _rc = useRuntimeConfig()
  const _smtpDefaults: Array<{ key: string; value: string }> = [
    { key: 'smtp_host', value: _smtp.host || (_rc.smtpHost as string) || '' },
    { key: 'smtp_port', value: _smtp.port ? String(_smtp.port) : ((_rc.smtpPort as string) || '587') },
    { key: 'smtp_secure', value: _smtp.secure !== undefined ? String(_smtp.secure) : ((_rc.smtpSecure as string) || 'false') },
    { key: 'smtp_user', value: _smtp.user || (_rc.smtpUser as string) || '' },
    { key: 'smtp_pass', value: _smtp.pass || (_rc.smtpPass as string) || '' },
    { key: 'smtp_from', value: _smtp.from || (_rc.smtpFrom as string) || '' },
    { key: 'dashboard_url', value: (_fileConfig?.app?.url) || (_rc.dashboardUrl as string) || '' },
  ]
  for (const setting of _smtpDefaults) {
    const _existing = await db('settings').where({ key: setting.key }).first()
    if (!_existing) await db('settings').insert(setting)
  }
  ...
  // ── migration: seed default password policy settings ──────────────────────
  const defaultPolicySettings = [
    { key: 'password_min_length', value: '8' },
    { key: 'password_require_uppercase', value: 'false' },
    { key: 'password_require_number', value: 'false' },
    { key: 'password_require_special', value: 'false' },
  ]
  ...
  // ── system_logs ────────────────────────────────────────────────────────────
  const hasSystemLogs = await db.schema.hasTable('system_logs')
  if (!hasSystemLogs) {
    await db.schema.createTable('system_logs', (table) => {
      table.increments('id').primary()
      table.string('level', 10).notNullable().defaultTo('error') // error | warn | info
  ...
  // ── log settings (migration) ─────────────────────────────────────────────
  const defaultLogSettings = [
    { key: 'log_retention_days', value: '7' },
    { key: 'log_purge_interval_hours', value: '24' },
  ]
  for (const setting of defaultLogSettings) {
    const exists = await db('settings').where({ key: setting.key }).first()
    if (!exists) await db('settings').insert(setting)
  }
}
