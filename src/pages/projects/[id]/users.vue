    ...
    >
      <template #body>
        <p class="text-gray-600 dark:text-gray-400">
          {{ t('users.remove_confirm', 'Remove') }} <strong>{{ deletingUser?.name }}</strong>
          {{ t('users.remove_from_project', 'from project') }} <strong>{{ currentProject?.name }}</strong>?
...
<script lang="ts" setup>
  ...
  watch(currentProject, (p) => {
    if (import.meta.client && !p) navigateTo('/projects')
  })
